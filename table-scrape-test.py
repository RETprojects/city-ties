...
# scrape cities from a table on Wikipedia & return a dataframe of cities (name & keywords)
def get_cities(title):
    html = wp.page(title, auto_suggest=False).html().encode("UTF-8")
    try: 
    ...
    city_df.drop(['City/Town', 'Country/Territory'], axis=1, inplace=True) # we don't need these columns now
    ...
    city_df.reset_index(drop=True, inplace=True) # if we dropped any rows, make sure that the indexing doesn't have gaps
    
    return city_df

if __name__ == '__main__': # for Windows compatibility

    city_df = get_cities("List of national capitals")
    ...
    for index, row in city_df.iterrows():
        ...
        city_df.loc[index, 'Kmeans'] = y_kmeans[index - 1]
    city_df = city_df.iloc[:, [0, 2, 1]] # reorder the columns for viewing cluster numbers conveniently

    # save the data
    city_df.to_csv('national_capitals_no_duplicates.csv', index=False)